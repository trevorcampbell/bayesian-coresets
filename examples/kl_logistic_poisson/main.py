...
pihat_noise = 5. #noise level (relative) for corrupting pihat
###############################
...
T_noisy = bc.MonteCarloFiniteTangentSpace(log_likelihood, lambda sz : np.random.multivariate_normal(muhat, Sighat, sz), projection_dim)
T_true = bc.MonteCarloFiniteTangentSpace(log_likelihood, lambda sz : np.random.multivariate_normal(mup, 9*Sigp, sz), projection_dim)
def tangent_space_factory(wts, idcs):
  if idcs.shape[0] > 0:
    ...
    muw, Sigw = mu0, Sig0
  return bc.MonteCarloFiniteTangentSpace(log_likelihood, lambda sz : np.random.multivariate_normal(muw, Sigw, sz), n_samples)
