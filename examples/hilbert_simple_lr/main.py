...
res = minimize(lambda mu : -log_joint(Z, mu, np.ones(Z.shape[0]))[0], Z.mean(axis=0), jac=lambda mu : -grad_log_joint(Z, mu, np.ones(Z.shape[0])).flatten())
#then find a quadratic expansion around the mode, and assume the distribution is Gaussian
mu = res.x
cov = -np.linalg.inv(hess_log_joint(Z, mu, np.ones(Z.shape[0])))[0, :, :]
...
coreset = bc.HilbertCoreset(Z, projectortsf) #do coreset construction using the discretized log-likelihood functions
