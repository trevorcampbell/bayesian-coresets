...
samples_using_code_for_weights = mcmc.sampler(dnm, curX, curY, mcmc_steps, stan_representation, weights=wts) 
#build the modified dataset so that we can run the unweighted version of sampler
for i in range(len(idcs)):
    toAdd = np.ones(weights[i]) * i #list i a number of times equal to the weight of the ith index
