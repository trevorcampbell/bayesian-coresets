  ...
  for i0 in [0.1, 1., 10.]:
    for ID in range(1,2):
      for alg in ['BPSVI']: #, 'SVI', "RAND", "PRIOR", "GIGAO", "GIGAR"]:
        for dnm in ['mnist2class_test']: #['fma', 'ds1.100', 'mnist2class_test']:
          c += 1
...
M = 50
SVI_step_sched = lambda itr : 0.05/(1.+itr)
BPSVI_step_sched = lambda m: lambda itr : (i0 + i0*(1.-m/M))/(1.+itr) # make step schedule potentially dependent on coreset size
n_subsample_opt = 200
...
#save results
f = open('results/largescale_adaptive_'+dnm+'_'+alg+'_results_'+str(i0)+'_'+str(ID)+'.pk', 'wb')
