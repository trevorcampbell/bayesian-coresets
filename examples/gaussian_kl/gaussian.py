...
#logpdf for single x, multiple mus
def gaussian_logpdf(x, mu, Sig):
  return -0.5*x.shape[0]*np.log(2*np.pi) - 0.5*np.linalg.slogdet(Sig)[1] - 0.5*( (x-mu).dot(np.linalg.inv(Sig))*(x-mu)).sum(axis=1)
