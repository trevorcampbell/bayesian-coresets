...
def test_tree_correctness():
  tests = [(N, D, dist) for N in [10, 1000] for D in [3, 10] for dist in ['gauss', 'bin']]
...
def check_tree_bounds(node, yw, y_yw):
  ...
  assert np.all((v**2).sum(axis=1)== 1.), "v is not normalized, norms = " + str((v**2).sum(axis=1))
